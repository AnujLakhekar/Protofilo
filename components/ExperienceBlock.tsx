...
const ExperienceBlock = ({ experiences: initialExperiences }: ExperienceBlockProps) => {
    const { data, isLoading, error } = useQuery({
        ...
        enabled: !initialExperiences, // Skip fetching if initialExperiences prop is passed
    });

    const experiencesList: Experience[] = initialExperiences || data?.message || [];

    if (isLoading && !initialExperiences && !error) return <p>Loading...</p>

    return (
        <div className="w-full max-w-2xl mx-auto px-2 sm:px-4 md:px-8">
            {experiencesList.map((experience: Experience, index: number) => (
